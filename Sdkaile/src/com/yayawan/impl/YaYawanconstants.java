		...
		Yayalog.loger("YaYawanconstantssdk支付");
         ...
         sdkManager.showPay(customPayParam, new OnPaymentListener() {
             ...
             public void paymentSuccess(PaymentCallbackInfo callbackInfo) {
                 ...
                 paySuce();
                 Toast("支付成功");
                 log("充值金额数：" +callbackInfo.money + " 消息提示：" + callbackInfo.msg);
             }
             ...
             public void paymentError(PaymentErrorMsg errorMsg) {
                 ...
                 payFail();
                 Toast("支付失败");
                 log("充值失败：code:" +errorMsg.code + "  ErrorMsg:" + errorMsg.msg +"  预充值的金额：" + errorMsg.money);
             }
         });
	}
...
//	        roleInfo.setServer_id("Server_id");
//	        roleInfo.setServer_name("server_Name");//测试用的
	        return roleInfo;
	    }
...
//		  RoleInfo roleInfo = initTestRoleInfo(roleId,roleName,roleLevel,zoneId,zoneName,roleCTime);
		Role_id = roleId;
		role_Name = roleName;
		Server_id = zoneId;
