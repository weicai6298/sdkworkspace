...
public class ChargerImpl implements YYWCharger {
	...
	public void createOrder(final Activity paramActivity) {
		...
		requestParams.addBodyParameter("meizu_uid", YaYawanconstants.uid);
		requestParams.addBodyParameter("buy_amount", "1");
		requestParams.addBodyParameter("pay_type", "0");
		requestParams.addBodyParameter("product_body", YYWMain.mOrder.goods);
		...
		requestParams.addBodyParameter("create_time", create_time);
		
		Log.i("tag","createOrder-create_time="+create_time);
		
		Yayalog.loger("meizu_uid:"+ YaYawanconstants.uid);
		Yayalog.loger("product_body:"+  YYWMain.mOrder.goods);
		Yayalog.loger("product_id:"+ YYWMain.mOrder.goods_id);
		Yayalog.loger("product_per_price:"+ "" + YYWMain.mOrder.money);
		Yayalog.loger("product_subject:"+ "购买"+YYWMain.mOrder.goods);
		Yayalog.loger("product_unit:"+ YYWMain.mOrder.goods);
		Yayalog.loger("total_price:"+ "" + YYWMain.mOrder.money);
					...
					public void onSuccess(ResponseInfo<String> arg0) {
						...
						try {
							...
							if (err_code == 0) {
								...
								time =  data.optString("create_time");
	...
	private void pay_run(final Activity paramActivity) {
		Log.i("tag","pay_runzhifu="+time);
		YaYawanconstants.pay(paramActivity, orderId ,sign,time);
	}
