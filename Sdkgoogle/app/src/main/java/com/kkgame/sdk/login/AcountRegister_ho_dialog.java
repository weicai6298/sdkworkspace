...
//	private ImageButton ib_mNotAgreedbox;
	...
	public void createDialog(final Activity mActivity) {
		...
		machineFactory.MachineButton(bt_mPhoneRegister, 232, 78, 0, "手機號註冊", 28,
		...
		if (KgameSdk.sdktype==3) {
			machineFactory.MachineButton(bt_mAccountRegister, 232, 78, 0, "用戶名註冊", 28,
					mRelativeLayout, 160, 0, 0, 0);
		}else {
			machineFactory.MachineButton(bt_mAccountRegister, 232, 78, 0, "用戶名註冊", 28,
		...
		// 用户名输入列
				...
				ll_mUser.setGravity(Gravity.CENTER);
						...
						et_mUser = machineFactory.MachineEditText(et_mUser, 0, MATCH_PARENT, 1,
								"用戶名：6-8位數字或者字母", 22, mLinearLayout, 0, 6, 0, 0);
				...
				et_mPassword = machineFactory.MachineEditText(et_mPassword, 0, MATCH_PARENT, 1,
										"密碼：6-8位數字或者字母", 22, mLinearLayout, 0, 4, 0, 0);
				...
				et_mRePassword = machineFactory.MachineEditText(et_mRePassword, 0, MATCH_PARENT, 1,
										"請再次輸入密碼", 22, mLinearLayout, 0, 4, 0, 0);
...
//		});
		...
		machineFactory.MachineButton(bt_mOk, MATCH_PARENT, 78, 0, "註冊", 36,
...
//				 }
				 if(!mPassword.equals(mRePassword)){
					 ...
					 Toast.makeText(mContext, "兩次輸入的密碼不壹致", Toast.LENGTH_SHORT)
						.show();
					 return;
				 }
				if (mName.equals("")) {
					Toast.makeText(mContext, "用戶名不能為空", Toast.LENGTH_SHORT)
							.show();
				} else if (mPassword.equals("")) {
					Toast.makeText(mContext, "密碼不能為空", Toast.LENGTH_SHORT)
							.show();
				} else if (mName.length() < 6) {
					Toast.makeText(mContext, "用戶名不能小於六位", Toast.LENGTH_SHORT)
							.show();
				} else if (mName.length() > 20) {
					Toast.makeText(mContext, "用戶名不能大於20位", Toast.LENGTH_SHORT)
							.show();
				} else if (mPassword.length() < 6) {
					Toast.makeText(mContext, "密碼不能小於六位", Toast.LENGTH_SHORT)
							.show();
				} else if (mPassword.length() > 20) {
					Toast.makeText(mContext, "密碼不能大於20位", Toast.LENGTH_SHORT)
							.show();
				} else {
					Utilsjf.creDialogpro(mActivity, "正在快速註冊...");
					...
					httpUtils.send(HttpMethod.POST,
							...
							new RequestCallBack<String>() {
								...
								public void onFailure(HttpException arg0,
									...
									Utilsjf.stopDialog();
									Toast.makeText(mActivity, "註冊失敗，請檢查網絡", 0)
											.show();
								}
								...
								public void onSuccess(
									...
									// TODO Auto-generated method stub
									Yayalog.loger("註冊返回信息" + result.result);
									Utilsjf.stopDialog();
									...
									if (user != null) {
										...
										UserDao.getInstance(mContext)
												.writeUser(
														...
														"123");
										AgentApp.mUser.password="";
										Yayalog.loger("登陸的uid："+user.toString());
										allDismiss();
